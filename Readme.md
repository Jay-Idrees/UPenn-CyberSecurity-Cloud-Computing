...
**6.** Now I want to create an inbound rule that will allow inbound from my specific ip - I can do this by going to [ip4.me](www.ip4.me) to discover my ip address. Under this inbound rule I can specify the service to be RDP (Remote desktop protocol- which it will automatically select to be TCP)  U Set up cryptographic **SSH keys-First** to access cloud servers, Passwords are weak because they can be brute forced. This is called `ground up security`. key commands: `ssh-keygen`, Do not enter any password and press enter twice,`cat ~/.ssh/id_rsa.pub`- to view the key. I did this on my laptop 
