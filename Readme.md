...
2. Create a **VNet**(Red-Team-VNet) and link it to the resource group(RedTeam). VNets have software version of their Virtyal Network Interface cards. The goal is to design a large network and then have a subnet inside of it (For large network- 10.0.0.0/16, for a subnet- 10.10.1.0/24). Usually the broarder net and 10.0.0.0/24 subet is created by default. 
 ...
 - Specify the IP address and subnet and disable the additional security features DoS if you do not want to pay extra
...
3. Setup a **network security group** (RedTeam-SG) - Add inboud security rule to block all traffic (deny)- It is equvilant of a network firewall. Imagine a firewall surrounding the VNET and between the virtual machines. Simply adding a NSG to the resource group will grant some basic set of security rules. They are inhound and outbound. What these do is allow inbound and outbound through a load balancer or within network freely, but blocks every connection from outside by default. Therefore if you are later going to want to connect to Virtual machines through the internet then we will have to make exceptions to these 'univerversal for outside' block rules. In other words, all the traffic will be blocked unless the bother the source and destination are within the network 

**4.** Add a new inbound **security rule** to configure the network security group just created - Block all traffic- This is the default deny rule that should get the highest priority (4096) - This is the default deny- Everything is blocked. We can also block all ports by specifying range like `0-65535`. I have created this rule before configuring any VMs in the network so that no one can access while I am configugring the network. 

5. **At this point I have a VNet protected by the security group that blocks all traffic from outside the network. No virtual machine is added yet, but the Goal is to create 3 machines in the same resource group**

**6.** Now I want to create an inbound rule that will allow inbound from my specific ip - I can do this by going to [ip4.me](www.ip4.me) to discover my ip address. Under this inbound rule I can specify the service to be RDP (Remote desktop protocol- which it will automatically select to be TCP) and select the action to allow. It is also important to understand about the priority. The higher the priority, the lower the number. For example the deny all inbound rule has the priority of 66550, and so if I want to create a new rule to allow an inbound exception for my home computer's ip address, then I will have to set this to a number that is below 65500 so that this rule is applied before the rule of blocking and my particular IP is granted access.

-  Set up cryptographic **SSH keys-First** to access cloud servers, Passwords are weak because they can be brute forced. Before setting this up you can also check if there are any pre-exisitng SSH keys This is called `ground up security`. key commands: `ssh-keygen`, Do not enter any password and press enter twice,`cat ~/.ssh/id_rsa.pub`- to view the key. I did this on my laptop. Note that the ssh keys are located in a hidden folder `.ssh` if I have a pre-existing key such as the github, then it is probably better that I create a new folder and move the old keys there an then generate a new id_rsa and id-rsa.pub keys. Alternatively you can also change the name. In this guide I have created a new ssh key in the default folder and the key has been created w/o password so I do not run into issues when connecting to the cloud later. `id_rsa` is the private key while `id_rsa.pub` is the public key. The one in the pub file is the one that should be place in the cloud settings. 

7. So far I only have a VNet-no VM, Create a new Virtual Machine in Azure using +Add: Specify the resource group(Red-Team), Call this first VM **Jump-Box-Provisioner VM**. Select  Be consistent with the region selected. I am on track to create 3 VMs - imp-these should share the same resource group and the region and the security group. The first VM is called Jump-box the other 2 Web-1 and  Wb-2


8. **Jump-Box-Provisioner VM settings**: Image: Ubuntu Server 18.04, Standard-B1s 1CPU and 1RAM (Jump-Box needs only 1 ram). Create a `username (RedAdmin)` and associate the SSH public key with it. Paste the public SSH key. I am not using password. Under the networking tab I can select the NSG settigns to be advance and then specify NSG-Team-NSG- that is select the security group that I have specified. ALternatively I can select the NSG setings to be basic and allow `SSH(22)` port for inbound -This is important as here I am allowing that an external ip can connect to the Jump-Box-Provisioner via the port 22. use advanced option for security group under the networking tab for custom configuration of the security group. You can also have the same username for all 3 machines

9. Create **2 more VMs (Web-1, Web-2)**. Same Resource group, Same Region, Same Network Security Group and same availability set. For the time being I will be using the same SSH key,later these will be replaced. For specification select "Standard-B1ms" for disks. Free Azure account only allows 4 virtual machines. The new VMs should have the **same availability set**. I have named this availability set `RedTeamAS` This is important so all the machines can be added to the load balancer later- Use the same availability set name. Select public IPs for these virtual machines as none. THe availability set specifies the update domain and the failure domain which means that the VMs that are in the same availability set can be rebooted together(update domain), while those in the failuer domain share the same database and power source. In other words, VMs in the same availability set have the same rebootability, database and power. In that sense it is used for grouping. Initially all the VMs will share the same SSH key (i-e Jump-box, Web-1 and Web-2). The new VMs should not have public IP addresses, unike Jump-box that will have a pubic ip
